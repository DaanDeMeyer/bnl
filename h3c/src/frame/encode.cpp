...
uint64_t frame::encoder::payload_size(const frame &frame) const noexcept
{
...
size_t frame::encoder::encoded_size(const frame &frame) const noexcept
{
  uint64_t payload_size = this->payload_size(frame);
...
std::error_code frame::encoder::encode(uint8_t *dest,
                                       size_t size,
                                       const frame &frame,
                                       size_t *encoded_size) const noexcept
{
  ...
  *encoded_size = 0;
  ...
  TRY_VARINT_ENCODE(util::to_underlying(frame.type_));
