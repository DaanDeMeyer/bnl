...
TEST_CASE("varint")
{
  ...
  SUBCASE("zero")
  {
    ...
    h3c::buffer encoded = encoder.encode(varint, ec);
  ...
  SUBCASE("max")
  {
    uint64_t varint = h3c::varint::max;
  ...
  {
    uint64_t varint = h3c::varint::max + 1;
  ...
  SUBCASE("decode: incomplete")
  {
    ...
    h3c::buffer incomplete = encoded.slice(encoded.size() - 1);
