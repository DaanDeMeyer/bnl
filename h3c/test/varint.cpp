...
TEST_CASE("varint")
{
  SUBCASE("zero")
  {
    ...
    H3C_ERROR error = h3c_varint_encode(dest.data(), dest.size(), n,
                                        &varint_size);
    ...
    error = h3c_varint_decode(dest.data(), dest.size(), &n, &varint_size);
  ...
  SUBCASE("encode: varint size")
  {
    ...
    size_t varint_size = 0;
    H3C_ERROR error = h3c_varint_encode(nullptr, 0, n, &varint_size);
  ...
  SUBCASE("decode: incomplete")
  {
    ...
    error = h3c_varint_decode(dest.data(), dest.size() - 1, &n, &varint_size);
